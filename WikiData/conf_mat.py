# compute recall, precision, f1 based on similarity matrix labels

import pandas as pd
import numpy as np

def conf_mat(dir):
    gnd_truths = open(dir+"mapping.txt", 'r').readlines()
    labels = dir+"similarity_matrix_label.csv"

    gnd_truths_pairs = set()
    for p in gnd_truths:
        plst = p.split(', ')
        gnd_truths_pairs.add((plst[0][1:], plst[1][:-2]))
